...
func _on_resources_reimported(resources: PackedStringArray) -> void:
	...
	if yaml_editor_instance and is_instance_valid(yaml_editor_instance):
		var open_files = yaml_editor_instance.get_open_files()
		for path in resources:
			if path.get_extension().to_lower() in ["yaml", "yml"] and path in open_files:
				# Reload the file
				yaml_editor_instance.file_manager.open_file(path)
