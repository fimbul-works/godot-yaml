...
func _ready():
	...
	for key in variants:
		test_variant_conversion(key, variants[key])
	...
	var parse_result := YAML.parse(yaml_result.get_data())
...
func test_variant_conversion(type_name: String, value: Variant) -> void:
