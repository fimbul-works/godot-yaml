...
var test_values = {
	...
	"max": Vector3i(2147483647, 2147483647, 2147483647),  # INT32_MAX
	"min": Vector3i(-2147483647, -2147483647, -2147483647)  # INT32_MIN
}

func _init():
	test_name = "🧩 Vector3i YAML Tests"
...
func test_basic_serialization() -> void:
	for name in test_values:
		...
		assert_roundtrip(parse_result, vec, is_vector3i_equal, name)
...
func test_container_forms() -> void:
	...
	assert_roundtrip(YAML.parse(map_result.get_data()), vec, is_vector3i_equal, "map form")
	assert_roundtrip(YAML.parse(seq_result.get_data()), vec, is_vector3i_equal, "sequence form")
...
func test_flow_styles() -> void:
	...
	assert_roundtrip(YAML.parse(flow_result.get_data()), vec, is_vector3i_equal, "flow style")
	assert_roundtrip(YAML.parse(block_result.get_data()), vec, is_vector3i_equal, "block style")
