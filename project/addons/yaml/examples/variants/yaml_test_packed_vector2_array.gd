extends YAMLTest
## Test suite for PackedVector2Array YAML serialization and styling

func _init():
	test_name = "🧩 PackedVector2Array YAML Tests"

# Helper function to create test arrays with various Vector2 patterns
func create_test_arrays() -> Dictionary:
	return {
		"empty": PackedVector2Array(),
		"single": PackedVector2Array([Vector2(1, 2)]),
		"basic": PackedVector2Array([
			Vector2(10, 20),
			Vector2(30, 40),
			Vector2(50, 60)
		]),
		...
		"mixed": PackedVector2Array([
			Vector2(1, -2),
			Vector2(-3, 4),
			Vector2(5, -6)
		]),
		"decimal": PackedVector2Array([
			Vector2(0.5, 1.5),
			Vector2(2.5, 3.5),
			Vector2(4.5, 5.5)
		]),
		"constants": PackedVector2Array([
			Vector2.ZERO,
			Vector2.ONE,
			Vector2.LEFT,
			Vector2.RIGHT,
		...
		"normalized": PackedVector2Array([
			Vector2(3, 4).normalized(),
			Vector2(-5, 12).normalized(),
			Vector2(0, 1).normalized()
		]),
		"large": create_large_vector2_array(50)
	}

# Helper to create a large array of Vector2s
func create_large_vector2_array(size: int) -> PackedVector2Array:
...
func test_item_flow_styles() -> void:
	...
	if !result.has_error():
		...
		assert_yaml_has_feature(result.get_data(), "- {", "Vector2 items use flow style")

		# Test roundtrip
		assert_roundtrip(YAML.parse(result.get_data()), vec_array, is_packed_vector2_array_equal, "array with flow items")

## Test parsing various Vector2Array formats
func test_parse_formats() -> void:
	print_rich("\nTesting parsing of different Vector2Array formats:")

	var test_formats = [
		"!PackedVector2Array []",  # Empty array
		"!PackedVector2Array [{x: 1, y: 2}]",  # Single element with map format
		"!PackedVector2Array [[1, 2]]",  # Single element with sequence format
		"!PackedVector2Array [{x: 1, y: 2}, {x: 3, y: 4}]",  # Multiple elements with map format
		"!PackedVector2Array [[1, 2], [3, 4]]",  # Multiple elements with sequence format
		"!PackedVector2Array [{x: 1.5, y: 2.5}, [3.5, 4.5]]"  # Mixed format (should still work)
	]
...
func test_large_array() -> void:
	...
	print_rich("\nTesting large Vector2Array handling:")
...
func test_roundtrip_with_styles() -> void:
	...
	if !parse_result.has_style():
		...
		var modified_array = PackedVector2Array()
		for vector in parsed_array:
			modified_array.append(vector)
		modified_array.append(Vector2(70, 80))
...
## Helper function to check if PackedVector2Array instances are equal
func is_packed_vector2_array_equal(a: PackedVector2Array, b: PackedVector2Array, epsilon: float = 0.00001) -> bool:
	if a.size() != b.size():
		return false
...
func format_array_for_display(array: PackedVector2Array) -> String:
	if array.size() == 0:
		return "[]"
