...
YAMLProjectionVariantConverter::YAMLProjectionVariantConverter(YAMLVariantConverterFactory *factory) :
		vec4_converter(factory->create_converter_as<YAMLVector4VariantConverter>(Variant::VECTOR4)) {
	ERR_FAIL_NULL(vec4_converter);
}

void YAMLProjectionVariantConverter::encode(ryml::NodeRef &node, const Variant &v, const YAMLStyle::View &style) const {
...
Variant YAMLProjectionVariantConverter::decode(const ryml::ConstNodeRef &node, YAMLParserContext *context) const {
	try {
		if (node.is_map()) {
