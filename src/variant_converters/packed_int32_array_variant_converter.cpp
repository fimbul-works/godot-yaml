	...
	for (size_t i = 0; i < size; ++i) {
		try {
			...
			if (value < INT32_MIN || value > INT32_MAX) {
				...
				throw YAMLException(vformat("Failed to decode PackedInt32Array value at index %d: Integer value out of range", static_cast<int64_t>(i)), node[i].location(*context->get_ryml_parser()));
			}
			...
			// NOTE: Cast size_t to int64_t for ARM64 macOS Variant compatibility
			throw YAMLException(vformat("Failed to decode PackedInt32Array value at index %d: %s", static_cast<int64_t>(i), e.what()), node[i].location(*context->get_ryml_parser()));
		}
	}
