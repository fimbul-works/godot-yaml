/**
 * @file vector2i_yaml.h
 * @brief Defines the Vector2iVariantConverter for YAML serialization of Godot Vector2i type.
 *
 * This file contains the Vector2iVariantConverter class which handles the conversion
...
/**
 * @class Vector2iVariantConverter
 * @brief Converter for Godot Vector2i type to/from YAML format.
 *
 * The Vector2iVariantConverter class provides functionality to:
...
private:
	 ...
	 */
	Vector2i decode_from_map(const ryml::ConstNodeRef &node, ParserContext *context) const;
	 ...
	 */
	Vector2i decode_from_sequence(const ryml::ConstNodeRef &node, ParserContext *context) const;
};
