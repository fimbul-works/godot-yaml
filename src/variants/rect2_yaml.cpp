...
void Rect2VariantConverter::encode(ryml::NodeRef &node, const Variant &v, const YAMLStyle::View &style) const {
	...
	vec2_converter->encode(position_node, rect.position, style.has_child("position") ? style.get_child("position") : YAMLStyle::View());
	vec2_converter->encode(size_node, rect.size, style.has_child("size") ? style.get_child("size") : YAMLStyle::View());
}
