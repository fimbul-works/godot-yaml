...
void YAML::Emitter::reset()
{
  ...
  current_style = YAMLStyle::View::create_view(nullptr);
}
...
void YAML::Emitter::emit_value(ryml::NodeRef& node, const Variant& value, const YAMLStyle::View& style)
{
  switch (value.get_type()) {
    ...
    default:
      if (VariantConverterRegistry::get_instance().has_converter(value.get_type())) {
        const auto* converter = VariantConverterRegistry::get_instance().get_converter(value.get_type());
...
void YAML::Emitter::emit_dictionary(ryml::NodeRef& node, const Dictionary& dict, const YAMLStyle::View& style)
{
  ...
  if (style.is_valid() && style.get_container_form() == YAMLStyle::FORM_MAP) {
    style.apply_flow_style(node);
  }
  ...
  for (int i = 0; i < keys.size(); i++) {
    ...
    YAMLStyle::View value_style = style.is_valid() ? style.get_child(String(key)) : YAMLStyle::View();
